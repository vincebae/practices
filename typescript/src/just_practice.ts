...
function showResult(status: Status, score: Score): void {
  const msg =
    status === "SUCCESS"
      ? score === 5
...
const withInnerFunction = (outer: string) => (inner: string) =>
  console.log(`Outer function received ${outer}, inner ${inner}`);
...
const sum2 = (arr: number[]): number => {
  const sum_internal = (acc: number, arr: number[]): number => {
    if (arr.length === 0) {
      return acc;
    }
    const [head, ...tail] = arr;
...
const sum3 = (arr: number[]): number => arr.reduce(add, 0);

console.log(sum3(exampleArray));

const sum4 = (arr: number[]): number => arr.reduceRight(add, 0);

console.log(sum4(exampleArray));

const addCurr = (x: number) => (y: number) => x + y;
const addOne = addCurr(1);
const addTwo = addCurr(2);

const multiCurr = (x: number) => (y: number) => x * y;
